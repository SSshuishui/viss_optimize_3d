...
static inline void ensure_dir(const std::string& path){ std::string cmd = "mkdir -p " + path; int rc = std::system(cmd.c_str()); (void)rc; }
...
static void write_txt_3cols_stream(std::ofstream& ofs,
  ...
  for (size_t i = 0; i < n; ++i) ofs << (double)a[i] << " " << (double)b[i] << " " << (double)c[i] << "\n";
}
...
static inline int half_to_full_neg_idx(int ih){ return half_to_full_pos_idx(ih) + UNIQUE_BASELINES_PER_T; }
                                        ...
                                        int N)
{
  ...
  for(int t=0; t<T; ++t){
    ...
    for(int j=0; j<UNIQUE_BASELINES_PER_T; ++j){
      ...
      if(!nearly_neg_pair(u[i0], u[i1]) || !nearly_neg_pair(v[i0], v[i1]) || !nearly_neg_pair(w[i0], w[i1])) return false;
    }
  }
  return true;
}
                                        ...
                                        std::vector<float2>& hViss_full)
{
  ...
  for(int ih=0; ih<N_half; ++ih){
    int ip = half_to_full_pos_idx(ih), in = half_to_full_neg_idx(ih);
...
static inline bool load_dcf_mb_bin(const std::string& path, DcfMbBinPayload& data){
  DcfMbBinHeader hdr{}; std::ifstream ifs(path, std::ios::binary); if(!ifs.is_open()) return false;
  ifs.read(reinterpret_cast<char*>(&hdr), sizeof(hdr)); if(!ifs.good()) return false;
  const char magic[8] = {'D','C','F','M','B','I','N','1'};
  ...
  data.lambda_m = hdr.lambda_m; data.bl_max = hdr.bl_max; data.dcf_days = (int)hdr.dcf_days; data.orbit_seed = hdr.orbit_seed;
  data.mb.resize((size_t)hdr.mb_len); data.dcf.resize((size_t)hdr.dcf_len);
...
static inline std::string default_dcf_bin_name(const std::string& btag, int dcf_days, uint64_t orbit_seed){
  std::ostringstream oss; oss << "dcf_mb_" << btag << "_days" << dcf_days << "_seed" << orbit_seed << ".bin"; return oss.str();
}
...
struct OrbitSegSlot {
  ...
  int t0 = 0, tlen = 0, segN = 0;
};

struct OrbitGenCtx {
  int dev = 0; cudaStream_t stream = nullptr; float lambda_m = 0.0f; int dayid = 1; uint64_t seed = 12345;
  int OrbitRes = 0, ProcessionCount = 0, segLen = 0, posnum = 0, max_tlen = 0, max_segN = 0;
