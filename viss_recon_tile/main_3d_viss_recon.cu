...
static constexpr int RECON_TILE_BL = 64;
...
int main(int argc,char** argv){
  ...
  std::cout<<"btag="<<btag<<" nside="<<nside<<" npix="<<npix
           ...
           <<" dcf_days="<<dcf_days<<"\n";
  std::cout<<"blockage="<<blockage<<"  [Stage-1 Viss=tilecone+blockage(always ON), Stage-2 Recon=direct 3D half-sym tilecone]\n";
  ...
  std::cout << "Generator logical GPU=" << gen_gpu_index
            << ", peer-staging-needed=" << (binfo.need_host_stage ? "YES" : "NO") << "\n";
  std::cout << "Baseline broadcast: gen_dev=" << gen_gpu_index
            << " need_host_stage=" << (binfo.need_host_stage ? 1 : 0) << "\n";
  ...
  for(int dayid=day_start; dayid<day_start+day_count; ++dayid){
    ...
    for(int s=0; s<segs; ++s){
      ...
      for(int gi=0; gi<G; ++gi){
        ...
        cudaStream_t stream=ctx[gi].compute_stream;
        GpuSegSlot &slot = ctx[gi].slots[cur];
        int grid = ctx[gi].ntile;

        if(blockage==1){
          recon_3d_direct_halfsym_tilecone_real<RECON_TILE_PIX_HOST, RECON_TILE_BL, true><<<grid, RECON_TILE_PIX_HOST, 0, stream>>>(
            ctx[gi].n_chunk,
            ctx[gi].d_l, ctx[gi].d_m, ctx[gi].d_n,
            ctx[gi].d_tile_cx, ctx[gi].d_tile_cy, ctx[gi].d_tile_cz,
            ctx[gi].d_tile_cosA, ctx[gi].d_tile_sinA, ctx[gi].ntile,
          ...
          recon_3d_direct_halfsym_tilecone_real<RECON_TILE_PIX_HOST, RECON_TILE_BL, false><<<grid, RECON_TILE_PIX_HOST, 0, stream>>>(
            ctx[gi].n_chunk,
            ctx[gi].d_l, ctx[gi].d_m, ctx[gi].d_n,
            ctx[gi].d_tile_cx, ctx[gi].d_tile_cy, ctx[gi].d_tile_cz,
            ctx[gi].d_tile_cosA, ctx[gi].d_tile_sinA, ctx[gi].ntile,
      ...
      std::cout << "[day " << dayid << " seg " << (s+1) << "/" << segs
                ...
                << " mode=direct3d_halfsym_tilecone\n";
    }
