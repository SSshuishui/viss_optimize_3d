...
static inline void compute_dcf_from_mb(const std::vector<long long>& mb, std::vector<float>& dcf){
  ...
  for(int p=1; p<=nr; ++p){
    float R = 0.5 * p + 0.25;
    float num = (2.0f/3.0f) * M_PI * (R*R*R - (R - 0.5f)*(R - 0.5f)*(R - 0.5f));
    float cnt = mb[p-1];
    dcf[p] = (cnt > 0) ? (num / cnt) : 0.0f;
  }
}
  ...
  for(int dayid=1; dayid<=dcf_days; ++dayid){
    ...
    for(int s=0; s<segs; ++s){
      ...
      for(int ih=0; ih<segN_half; ++ih){
        ...
        float vv = sg.h_v[i];
        float ww = sg.h_w[i];
        float bll = sqrtf(uu*uu + vv*vv + ww*ww);
        int bin = (int)ceilf((bll - 0.25f) / 0.5f);
    ...
    orbit_gen_destroy(gen);
    if(dayid == 1 || dayid == dcf_days || (dayid % 50) == 0){
