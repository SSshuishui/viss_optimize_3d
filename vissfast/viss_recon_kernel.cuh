...
static inline void launch_vissfast_halfsym_tilecone_runtime(
    ...
    const float* d_x1, const float* d_y1, const float* d_z1, const float* d_invn1,
    const float* d_x2, const float* d_y2, const float* d_z2, const float* d_invn2,
    int N_half, float cosphi, float2* d_Vpart)
{
  if(tile_pix == 512){
    viss_partial_all_halfsym_tilecone<512,DO_BLOCKAGE><<<gridV,blockV,shmem,stream>>>(d_B,d_l,d_m,d_n,d_nm1,n_chunk,d_tile_cx,d_tile_cy,d_tile_cz,d_tile_cosA,d_tile_sinA,ntile,d_u,d_v,d_w,d_x1,d_y1,d_z1,d_invn1,d_x2,d_y2,d_z2,d_invn2,N_half,cosphi,d_Vpart);
  }else{
    viss_partial_all_halfsym_tilecone<256,DO_BLOCKAGE><<<gridV,blockV,shmem,stream>>>(d_B,d_l,d_m,d_n,d_nm1,n_chunk,d_tile_cx,d_tile_cy,d_tile_cz,d_tile_cosA,d_tile_sinA,ntile,d_u,d_v,d_w,d_x1,d_y1,d_z1,d_invn1,d_x2,d_y2,d_z2,d_invn2,N_half,cosphi,d_Vpart);
  }
}
    ...
    float* __restrict__ pair_weight_half)
{
  ...
  int i = half_to_full_pos_idx_dev(ih);
  float uu = u[i], vv = v[i], ww = w[i];
    ...
    float* __restrict__ Cacc)
{
  ...
  for(int b0 = 0; b0 < N_half; b0 += TILE_BL){
    ...
    for(int t = lane; t < tileN; t += blockDim.x){
      int ih = b0 + t;
      int fi = half_to_full_pos_idx_dev(ih);
      su[t]   = u[fi];
      sv[t]   = v[fi];
      sw[t]   = w[fi];
      sx1[t]  = x1[fi];  sy1[t]  = y1[fi];  sz1[t]  = z1[fi];  sin1[t] = invn1[fi];
      sx2[t]  = x2[fi];  sy2[t]  = y2[fi];  sz2[t]  = z2[fi];  sin2[t] = invn2[fi];
    ...
    if constexpr (DO_BLOCKAGE){
      if(lane < tileN){
        float dotc1 = tcx * (sx1[lane] * sin1[lane]) + tcy * (sy1[lane] * sin1[lane]) + tcz * (sz1[lane] * sin1[lane]);
        float dotc2 = tcx * (sx2[lane] * sin2[lane]) + tcy * (sy2[lane] * sin2[lane]) + tcz * (sz2[lane] * sin2[lane]);
        bool av1=false, ah1=false, av2=false, ah2=false;
        classify_tile_cone(dotc1, tcosA, tsinA, cosphi, av1, ah1);
        classify_tile_cone(dotc2, tcosA, tsinA, cosphi, av2, ah2);
        sallv1[lane] = (unsigned char)(av1 ? 1 : 0);
        sallh1[lane] = (unsigned char)(ah1 ? 1 : 0);
        sallv2[lane] = (unsigned char)(av2 ? 1 : 0);
        sallh2[lane] = (unsigned char)(ah2 ? 1 : 0);
      }
    }
    __syncthreads();
    ...
    if(active){
      ...
      for(int t = 0; t < tileN; ++t){
        ...
        if constexpr (DO_BLOCKAGE){
          if(sallh1[t] || sallh2[t]) continue;

          if(!sallv1[t]){
            float c1 = (lp * sx1[t] + mp * sy1[t] + npv * sz1[t]) * sin1[t];
            if(c1 < cosphi) continue;
          }
          if(!sallv2[t]){
            float c2 = (lp * sx2[t] + mp * sy2[t] + npv * sz2[t]) * sin2[t];
            if(c2 < cosphi) continue;
          }
        }

        float phase = TWO_PI * fmaf(sw[t], npv, fmaf(sv[t], mp, su[t] * lp));
        float s, c;
