    ...
    std::vector<long long>& mb)
{
  ...
  for(int dayid=1; dayid<=dcf_days; ++dayid){
    ...
    orbit_gen_make_segment_async(gen, 0, seg_t0[0], seg_tlen[0], false);
    if(segs > 1) orbit_gen_make_segment_async(gen, 1, seg_t0[1], seg_tlen[1], false);
    ...
    for(int s=0; s<segs; ++s){
      ...
      int segN = sg.segN;
      int segN_half = sg.tlen * UNIQUE_BASELINES_PER_T;
      if(segN <= 0 || segN_half <= 0) continue;

      if(!validate_uvw_halfsym_layout_ptr(sg.h_u, sg.h_v, sg.h_w, segN)){
        std::cerr << "ERROR: dcf scan uvw half-symmetry failed at day=" << dayid
                  << " seg=" << s << "\n";
      ...
      for(int ih=0; ih<segN_half; ++ih){
        ...
        double uu = (double)sg.h_u[i];
        double vv = (double)sg.h_v[i];
        double ww = (double)sg.h_w[i];
      ...
      if(s + 2 < segs){
        orbit_gen_make_segment_async(gen, cur, seg_t0[s+2], seg_tlen[s+2], false);
      }
    }
