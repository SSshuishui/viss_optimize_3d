...
static inline double cuda_event_elapsed_s(cudaEvent_t start, cudaEvent_t stop){
  cudaError_t q = cudaEventQuery(stop);
...
static bool load_B_auto(const std::string& sky_dir,
  ...
  const std::string b2 = sky_dir + "/B.txt";
...
static void phase_correct_viss_halfsym_host(float2* Viss_half, const float* w_half, int N_half){
...
static void expand_baseline_halfsym_to_full_triplets(const float* a_half,
                                                     const float* b_half,
                                                     const float* c_half,
                                                     int N_half,
                                                     bool negate,
...
{
  int N_full = N_half * 2;
  a_full.assign((size_t)N_full, 0.0f);
  b_full.assign((size_t)N_full, 0.0f);
  c_full.assign((size_t)N_full, 0.0f);
  for(int ih=0; ih<N_half; ++ih){
    int ip = half_to_full_pos_idx(ih);
    int in = half_to_full_neg_idx(ih);
    a_full[ip] = a_half[ih];
...
static void expand_baseline_halfsym_swap_to_full_triplets(const float* a1_half,
                                                          const float* b1_half,
                                                          const float* c1_half,
                                                          const float* a2_half,
                                                          const float* b2_half,
                                                          const float* c2_half,
                                                          int N_half,
                                                          std::vector<float>& a1_full,
                                                          std::vector<float>& b1_full,
                                                          std::vector<float>& c1_full,
                                                          std::vector<float>& a2_full,
                                                          std::vector<float>& b2_full,
                                                          std::vector<float>& c2_full)
{
  int N_full = N_half * 2;
  a1_full.assign((size_t)N_full, 0.0f);
  b1_full.assign((size_t)N_full, 0.0f);
  c1_full.assign((size_t)N_full, 0.0f);
  a2_full.assign((size_t)N_full, 0.0f);
  b2_full.assign((size_t)N_full, 0.0f);
  c2_full.assign((size_t)N_full, 0.0f);
  for(int ih=0; ih<N_half; ++ih){
    int ip = half_to_full_pos_idx(ih);
    int in = half_to_full_neg_idx(ih);
    a1_full[ip] = a1_half[ih];
    b1_full[ip] = b1_half[ih];
    c1_full[ip] = c1_half[ih];
    a2_full[ip] = a2_half[ih];
    b2_full[ip] = b2_half[ih];
    c2_full[ip] = c2_half[ih];

    a1_full[in] = a2_half[ih];
    b1_full[in] = b2_half[ih];
    c1_full[in] = c2_half[ih];
    a2_full[in] = a1_half[ih];
    b2_full[in] = b1_half[ih];
    c2_full[in] = c1_half[ih];
  }
}

struct DcfMbBinPayload {
  std::string btag;
  float lambda_m = 0.0f;
  float bl_max = 0.0f;
  int dcf_days = 0;
...
static inline bool save_dcf_mb_bin(const std::string& path, const DcfMbBinPayload& data){
  DcfMbBinHeader hdr{};
  const char magic[8] = {'D','C','F','M','B','I','N','1'};
  std::memcpy(hdr.magic, magic, 8);
  hdr.version = 1;
  std::snprintf(hdr.btag, sizeof(hdr.btag), "%s", data.btag.c_str());
  hdr.lambda_m = data.lambda_m;
  hdr.bl_max = data.bl_max;
  hdr.dcf_days = data.dcf_days;
  hdr.orbit_seed = data.orbit_seed;
  hdr.mb_len = (uint64_t)data.mb.size();
  hdr.dcf_len = (uint64_t)data.dcf.size();
  std::ofstream ofs(path, std::ios::binary);
  if(!ofs.is_open()) return false;
  ofs.write(reinterpret_cast<const char*>(&hdr), sizeof(hdr));
  if(hdr.mb_len) ofs.write(reinterpret_cast<const char*>(data.mb.data()), (std::streamsize)(hdr.mb_len * sizeof(long long)));
  if(hdr.dcf_len) ofs.write(reinterpret_cast<const char*>(data.dcf.data()), (std::streamsize)(hdr.dcf_len * sizeof(float)));
  return ofs.good();
}

static inline bool load_dcf_mb_bin(const std::string& path, DcfMbBinPayload& data){
  DcfMbBinHeader hdr{};
  std::ifstream ifs(path, std::ios::binary);
  if(!ifs.is_open()) return false;
  ...
  const char magic[8] = {'D','C','F','M','B','I','N','1'};
  if(std::memcmp(hdr.magic, magic, 8) != 0 || hdr.version != 1) return false;
  data = DcfMbBinPayload{};
  data.btag = std::string(hdr.btag, hdr.btag + strnlen(hdr.btag, sizeof(hdr.btag)));
  data.lambda_m = hdr.lambda_m;
  data.bl_max = hdr.bl_max;
  data.dcf_days = (int)hdr.dcf_days;
  data.orbit_seed = hdr.orbit_seed;
  data.mb.resize((size_t)hdr.mb_len);
  data.dcf.resize((size_t)hdr.dcf_len);
  if(hdr.mb_len) ifs.read(reinterpret_cast<char*>(data.mb.data()), (std::streamsize)(hdr.mb_len * sizeof(long long)));
  if(hdr.dcf_len) ifs.read(reinterpret_cast<char*>(data.dcf.data()), (std::streamsize)(hdr.dcf_len * sizeof(float)));
  return ifs.good() || ifs.eof();
}

static inline std::string default_dcf_bin_name(const std::string& btag, int dcf_days, uint64_t orbit_seed){
  std::ostringstream oss;
  oss << "dcf_mb_" << btag << "_days" << dcf_days << "_seed" << orbit_seed << ".bin";
  return oss.str();
}
struct OrbitSegSlot {
  float *d_pos=nullptr;
