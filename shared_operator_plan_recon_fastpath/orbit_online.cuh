...
__global__
void k_compute_uvw_xyz_half(const float* pos, int posnum, float lambda_m,
                            float* u, float* v, float* w,
                            float* x1, float* y1, float* z1,
                            float* x2, float* y2, float* z2)
{
  const int amount = SATNUM * (SATNUM - 1) / 2;
  const int outN = amount * posnum;
  const int outIdx = blockIdx.x * blockDim.x + threadIdx.x;
  if (outIdx >= outN) return;

  const int timeIdx = outIdx / amount;
  const int k = outIdx - timeIdx * amount;
  const int m = d_pair_m[k];
  const int n = d_pair_n[k];
  ...
  // Keep the same ordering as the former first half of the full signed layout.
  // That former first half used sgn=-1 and endpoints (x1,x2)=(n,m).
  u[outIdx] = -(xn - xm) * 1e3f / lambda_m;
  v[outIdx] = -(yn - ym) * 1e3f / lambda_m;
  w[outIdx] = -(zn - zm) * 1e3f / lambda_m;
...
void orbit_gen_prepare_day(OrbitGenCtx& gen, int dayid){
  ...
  {
    uint64_t day_seed = gen.seed ^ (uint64_t)dayid * 0x9E3779B97F4A7C15ULL;
...
void orbit_gen_init(OrbitGenCtx& gen, int dev, float lambda_m, int dayid, int max_tlen, uint64_t seed){
  ...
  gen.max_segN_half = max_tlen * UNIQUE_BASELINES_PER_T;
  ...
  for(int slot=0; slot<2; ++slot){
    ...
    CHECK_CUDA(cudaMalloc(&sg.d_u,  (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&sg.d_v,  (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&sg.d_w,  (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&sg.d_x1, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&sg.d_y1, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&sg.d_z1, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&sg.d_x2, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&sg.d_y2, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&sg.d_z2, (size_t)gen.max_segN_half * sizeof(float)));

    CHECK_CUDA(cudaMallocHost(&sg.h_u,  (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMallocHost(&sg.h_v,  (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMallocHost(&sg.h_w,  (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMallocHost(&sg.h_x1, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMallocHost(&sg.h_y1, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMallocHost(&sg.h_z1, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMallocHost(&sg.h_x2, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMallocHost(&sg.h_y2, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaMallocHost(&sg.h_z2, (size_t)gen.max_segN_half * sizeof(float)));
    CHECK_CUDA(cudaEventCreate(&sg.gen_start));
...
void orbit_gen_make_segment_async(OrbitGenCtx& gen, int slot_idx, int t0, int tlen, bool copy_all_host){
  ...
  sg.t0 = t0;
  sg.tlen = tlen;
  sg.segN_half = tlen * UNIQUE_BASELINES_PER_T;
  sg.segN = tlen * SIGNED_BASELINES_PER_T;

  const int BS = 256;
  CHECK_CUDA(cudaEventRecord(sg.gen_start, gen.stream));
  int GS = (tlen + BS - 1) / BS;
  ...
  GS = (sg.segN_half + BS - 1) / BS;
  k_compute_uvw_xyz_half<<<GS, BS, 0, gen.stream>>>(sg.d_pos, tlen, gen.lambda_m,
  ...
  if(copy_all_host){
    CHECK_CUDA(cudaMemcpyAsync(sg.h_u, sg.d_u, (size_t)sg.segN_half*sizeof(float), cudaMemcpyDeviceToHost, gen.stream));
    CHECK_CUDA(cudaMemcpyAsync(sg.h_v, sg.d_v, (size_t)sg.segN_half*sizeof(float), cudaMemcpyDeviceToHost, gen.stream));
    CHECK_CUDA(cudaMemcpyAsync(sg.h_w, sg.d_w, (size_t)sg.segN_half*sizeof(float), cudaMemcpyDeviceToHost, gen.stream));
    CHECK_CUDA(cudaMemcpyAsync(sg.h_x1, sg.d_x1, (size_t)sg.segN_half*sizeof(float), cudaMemcpyDeviceToHost, gen.stream));
    CHECK_CUDA(cudaMemcpyAsync(sg.h_y1, sg.d_y1, (size_t)sg.segN_half*sizeof(float), cudaMemcpyDeviceToHost, gen.stream));
    CHECK_CUDA(cudaMemcpyAsync(sg.h_z1, sg.d_z1, (size_t)sg.segN_half*sizeof(float), cudaMemcpyDeviceToHost, gen.stream));
    CHECK_CUDA(cudaMemcpyAsync(sg.h_x2, sg.d_x2, (size_t)sg.segN_half*sizeof(float), cudaMemcpyDeviceToHost, gen.stream));
    CHECK_CUDA(cudaMemcpyAsync(sg.h_y2, sg.d_y2, (size_t)sg.segN_half*sizeof(float), cudaMemcpyDeviceToHost, gen.stream));
    CHECK_CUDA(cudaMemcpyAsync(sg.h_z2, sg.d_z2, (size_t)sg.segN_half*sizeof(float), cudaMemcpyDeviceToHost, gen.stream));
  }
...
void orbit_gen_destroy(OrbitGenCtx& gen){
  ...
  for(int slot=0; slot<2; ++slot){
    ...
    if(sg.gen_start) cudaEventDestroy(sg.gen_start);
...
__global__ void normalize3_inplace_kernel(float* x,float* y,float* z,int n){
  int i=blockIdx.x*blockDim.x+threadIdx.x;
  if(i<n){
    float a=x[i], b=y[i], c=z[i];
    float inv = rsqrtf(fmaxf(a*a+b*b+c*c, 1.0e-30f));
    x[i]=a*inv; y[i]=b*inv; z[i]=c*inv;
  }
}


// -------- online DCF accumulation (half-sym) --------


static inline void compute_dcf_from_mb(const std::vector<long long>& mb, std::vector<float>& dcf){
  int nr = (int)mb.size();
  dcf.assign((size_t)nr + 1, 0.0f);
  dcf[0] = 4.0f / (float)M_PI;
  for(int p=1; p<=nr; ++p){
...
static inline bool accumulate_mb_from_online_orbit(
    int gen_dev,
    float lambda_m,
    float bl_max,
    int dcf_days,
    ...
    std::vector<long long>& mb)
{
  ...
  for(int k=0; k<segs; ++k){
    ...
    seg_tlen[k] = len;
    curT += len;
    max_tlen = std::max(max_tlen, len);
  }

  int nr = (int)std::ceil(bl_max / lambda_m * 2.0f);
  mb.assign(nr, 0);

  for(int dayid=1; dayid<=dcf_days; ++dayid){
    OrbitGenCtx gen;
    orbit_gen_init(gen, gen_dev, lambda_m, dayid, max_tlen, orbit_seed);

    orbit_gen_make_segment_async(gen, 0, seg_t0[0], seg_tlen[0], true);
    if(segs > 1) orbit_gen_make_segment_async(gen, 1, seg_t0[1], seg_tlen[1], true);

    for(int s=0; s<segs; ++s){
      int cur = s & 1;
      orbit_gen_wait_slot(gen, cur);
      OrbitSegSlot &sg = gen.slots[cur];
      int segN = sg.segN;
      int segN_half = sg.segN_half;
      if(segN <= 0 || segN_half <= 0) continue;

      for(int ih=0; ih<segN_half; ++ih){
        double uu = (double)sg.h_u[ih];
        double vv = (double)sg.h_v[ih];
        double ww = (double)sg.h_w[ih];
        double bll = std::sqrt(uu*uu + vv*vv + ww*ww);
        int bin = (int)std::ceil((bll - 0.25) / 0.5);
        if(bin >= 1 && bin <= nr) mb[bin - 1] += 2;
      }

      if(s + 2 < segs){
        orbit_gen_make_segment_async(gen, cur, seg_t0[s+2], seg_tlen[s+2], true);
      }
    }

    orbit_gen_destroy(gen);
    if(dayid == 1 || dayid == dcf_days || (dayid % 10) == 0){
      std::cout << "[dcf] scanned day " << dayid << "/" << dcf_days << "\n";
    }
  }
