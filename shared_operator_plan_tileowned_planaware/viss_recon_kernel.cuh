...
#ifndef OP_RECON_TASK_BL_VALUE
#define OP_RECON_TASK_BL_VALUE 128
#endif
#ifndef OP_RECON_PLAN_BL_CHUNK_VALUE
#define OP_RECON_PLAN_BL_CHUNK_VALUE 512
#endif
#ifndef OP_RECON_TASK_TILE_PIX_VALUE
#define OP_RECON_TASK_TILE_PIX_VALUE 256
#endif
#ifndef OP_RECON_DEBUG_CHECK_FINITE
#define OP_RECON_DEBUG_CHECK_FINITE 0
#endif
#ifndef OP_RECON_SKIP_ZERO_WEIGHTED_VISS
#define OP_RECON_SKIP_ZERO_WEIGHTED_VISS 0
#endif
#ifndef OP_RECON_USE_TILEOWNED_PLAN
#define OP_RECON_USE_TILEOWNED_PLAN 1
#endif

static constexpr int OP_RECON_TASK_BL = OP_RECON_TASK_BL_VALUE;
static constexpr int OP_RECON_PLAN_BL_CHUNK = OP_RECON_PLAN_BL_CHUNK_VALUE;
static constexpr int OP_RECON_TASK_TILE_PIX = OP_RECON_TASK_TILE_PIX_VALUE;

#if OP_RECON_USE_TILEOWNED_PLAN
#define OP_RECON_MODE_LABEL "direct3d_halfsym_tileowned_planaware"
#else
#define OP_RECON_MODE_LABEL "direct3d_halfsym_tasklist_balance"
#endif
...
struct OperatorPlanBuffer {
  ...
  // Optional third-stage plan-aware tile-owned execution path.
  // Instead of launching one block per (baseline-block, sky-tile) task,
  // we compact only the sky tiles that have at least one non-hidden task
  // in the current baseline chunk. A CUDA block then owns one sky tile
  // and walks the visible/mixed baseline blocks for that tile.
  unsigned char* d_active_tile_flags=nullptr;
  int* d_active_tiles=nullptr;
  int* d_num_active_tiles=nullptr;
  int* h_num_active_tiles=nullptr;

  void* d_select_temp_vv=nullptr;
  void* d_select_temp_mixed=nullptr;
  void* d_select_temp_active_tiles=nullptr;
  size_t select_temp_vv_bytes=0;
  size_t select_temp_mixed_bytes=0;
  size_t select_temp_active_tiles_bytes=0;
...
struct GpuCtx {
  ...
  float* d_n=nullptr;
  float* d_nm1=nullptr;
  float* d_l_recon=nullptr;
  float* d_m_recon=nullptr;
  float* d_n_recon=nullptr;
  int ntile_viss=0;
  int ntile_recon=0;
...
// Apply the half-symmetry / DCF pair weight once per visibility sample.
// This removes pair-weight loads, multiply, and finite checks from the hot
// per-pixel recon loops. Non-finite visibility samples are sanitized here,
// preserving the previous "skip bad z" behavior at much lower frequency.
__global__ void apply_pair_weight_to_viss_half_kernel(
    float2* __restrict__ Viss_half,
    ...
    int N_half)
{
  ...
  float pw = pair_weight_half[ih];
  float2 z = Viss_half[ih];
  if(!(pw > 0.0f) || !isfinite(pw) || !isfinite(z.x) || !isfinite(z.y)){
    Viss_half[ih] = make_float2(0.0f, 0.0f);
  } else {
    Viss_half[ih] = make_float2(z.x * pw, z.y * pw);
  }
}
    ...
    float* __restrict__ Cacc)
{
  ...
  if(active){
    ...
    for(int t=0; t<tileBL; ++t){
      float2 z = sV[t];
#if OP_RECON_SKIP_ZERO_WEIGHTED_VISS
      if(z.x == 0.0f && z.y == 0.0f) continue;
#endif
#if OP_RECON_DEBUG_CHECK_FINITE
...
__global__ void build_active_tile_flags_from_recon_task_flags_kernel(
    int ntile,
    int nblocks,
    const unsigned char* __restrict__ vv_flags,
    const unsigned char* __restrict__ mixed_flags,
    unsigned char* __restrict__ active_tile_flags)
{
  int tile = blockIdx.x * blockDim.x + threadIdx.x;
  if(tile >= ntile) return;
  unsigned char active = 0;
  for(int blk=0; blk<nblocks; ++blk){
    int task = blk * ntile + tile;
    if(vv_flags[task] || mixed_flags[task]){
      active = 1;
      break;
    }
  }
  active_tile_flags[tile] = active;
}

template<int TILE_PIX, int TILE_BL>
__global__ void recon_3d_direct_planaware_tileowned_real(
    long long n_chunk,
    const float* __restrict__ l,
    const float* __restrict__ m,
    const float* __restrict__ n,
    ...
    const unsigned char* __restrict__ vv_flags,
    const unsigned char* __restrict__ mixed_flags,
    const int* __restrict__ active_tiles,
    int num_active_tiles,
    float* __restrict__ Cacc)
{
  int active_tile_idx = blockIdx.x;
  if(active_tile_idx >= num_active_tiles) return;
  int tile = active_tiles[active_tile_idx];
  if(tile < 0 || tile >= ntile) return;
  ...
  for(int blk=0; blk<nblocks; ++blk){
    int task = blk * ntile + tile;
    unsigned char is_vv = vv_flags[task];
    unsigned char is_mixed = mixed_flags[task];
    if(!is_vv && !is_mixed) continue;

    int base = b0 + blk * TILE_BL;
    int tileBL = min(TILE_BL, chunk_n - blk * TILE_BL);
    if(tileBL <= 0) continue;
    ...
    if(lane < tileBL){
      bool allv=false, allh=false;
      operator_classify_pair_tile_dirs(
          sdx1[lane],sdy1[lane],sdz1[lane],
          sdx2[lane],sdy2[lane],sdz2[lane],
          tcx,tcy,tcz,tcosA,tsinA,cosphi,allv,allh);
      if(allv){
        int dst = atomicAdd(&nvv, 1);
        svv_idx[dst] = (unsigned short)lane;
      } else if(!allh){
    ...
    if(active_pix){
      ...
      for(int ii=0; ii<nmix; ++ii){
        ...
        if(!isfinite(z.x) || !isfinite(z.y)) continue;
#endif
        if(!operator_point_visible_pair_dirs(
              sdx1[t],sdy1[t],sdz1[t],sdx2[t],sdy2[t],sdz2[t],lp,mp,npv,cosphi)) continue;
        float phase = operator_adjoint_phase(su[t], sv[t], sw[t], lp, mp, npv);
        float s, c; sincos_fast(phase, &s, &c);
        acc_re += z.x * c - z.y * s;
      }
...
#if OP_RECON_USE_TILEOWNED_PLAN
  {
    int grid_tiles = (gctx.ntile_recon + 255) / 256;
    build_active_tile_flags_from_recon_task_flags_kernel<<<grid_tiles,256,0,stream>>>(
        gctx.ntile_recon, nblocks, plan.d_vv_flags, plan.d_mixed_flags, plan.d_active_tile_flags);
    CHECK_CUDA(cudaPeekAtLastError());

    cub::CountingInputIterator<int> tile_counting(0);
    CHECK_CUDA(cub::DeviceSelect::Flagged(plan.d_select_temp_active_tiles, plan.select_temp_active_tiles_bytes,
        tile_counting, plan.d_active_tile_flags, plan.d_active_tiles, plan.d_num_active_tiles,
        gctx.ntile_recon, stream));
    CHECK_CUDA(cudaMemcpyAsync(plan.h_num_active_tiles, plan.d_num_active_tiles, sizeof(int), cudaMemcpyDeviceToHost, stream));
  }
#else
  {
...
#if OP_RECON_USE_TILEOWNED_PLAN
    {
      int num_active_tiles = *plan.h_num_active_tiles;
      stats.vv_tasks += (long long)num_active_tiles;
      if(num_active_tiles == 0){
        stats.zero_task_chunks += 1;
      } else {
        stats.mixed_chunks += 1;
        recon_3d_direct_planaware_tileowned_real<OP_RECON_TASK_TILE_PIX, OP_RECON_TASK_BL><<<num_active_tiles, OP_RECON_TASK_TILE_PIX, 0, cstream>>>(
            gctx.recon_n_chunk,
            gctx.d_l_recon, gctx.d_m_recon, gctx.d_n_recon,
            gctx.d_tile_cx_recon, gctx.d_tile_cy_recon, gctx.d_tile_cz_recon,
            ...
            b0, cn, cosphi,
            plan.d_vv_flags, plan.d_mixed_flags,
            plan.d_active_tiles, num_active_tiles,
            gctx.d_Cacc);
        CHECK_CUDA(cudaPeekAtLastError());
      }
    }
#else
    {
      ...
      if(num_vv > 0){
        recon_3d_direct_tasklist_vv_real<OP_RECON_TASK_TILE_PIX, OP_RECON_TASK_BL><<<num_vv, OP_RECON_TASK_TILE_PIX, 0, cstream>>>(
            gctx.recon_n_chunk, gctx.d_l_recon, gctx.d_m_recon, gctx.d_n_recon, gctx.ntile_recon,
            slot.d_u, slot.d_v, slot.d_w,
            slot.d_Viss,
      ...
      if(num_mixed > 0){
        recon_3d_direct_tasklist_mixed_real<OP_RECON_TASK_TILE_PIX, OP_RECON_TASK_BL><<<num_mixed, OP_RECON_TASK_TILE_PIX, 0, cstream>>>(
            gctx.recon_n_chunk, gctx.d_l_recon, gctx.d_m_recon, gctx.d_n_recon,
            gctx.d_tile_cx_recon, gctx.d_tile_cy_recon, gctx.d_tile_cz_recon,
            gctx.d_tile_cosA_recon, gctx.d_tile_sinA_recon, gctx.ntile_recon,
            slot.d_u, slot.d_v, slot.d_w,
            slot.d_x1, slot.d_y1, slot.d_z1, slot.d_x2, slot.d_y2, slot.d_z2,
